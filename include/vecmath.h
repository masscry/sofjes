...
namespace sj {
  ...
  template<typename T>
  inline T SqrDistance(vec2_t<T> a, vec2_t<T> b) {
    T dx = a.x - b.x;
    T dy = a.y - b.y;
    return dx*dx + dy*dy;
  }
  ...
  template<typename T>
  inline mat_t<T> operator * (mat_t<T> a, mat_t<T> b) {
    return mat_t<T> {
      {
  ...
  template<typename T>
  inline vec2_t<T> operator * (mat_t<T> mat, vec2_t<T> vec) {
    return vec2_t<T>{
  ...
  template<typename T>
  inline vec2_t<T> operator * (vec2_t<T> a, T b) {
  ...
  template<typename T>
  inline mat_t<T> RotationMatrix(T angle) {
  ...
  template<typename T>
  inline T InverseDeterminant(mat_t<T> mat) {
    return ((T) 1.0) / (mat.v[1][0]*mat.v[0][1] - mat.v[0][0]*mat.v[1][1]);
  }

  template<typename T>
  inline mat_t<T> Inverse(mat_t<T> mat) {
